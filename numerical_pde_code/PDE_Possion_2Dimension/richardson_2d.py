...
if __name__ == "__main__":
    ...
    # 求解
    X, Y, U_num1 = poisson_2d_jacobi(psi, f, xl, xr, yb, yt, m1, m2)

    _, _, U_num2 = poisson_2d_jacobi(psi, f, xl, xr, yb, yt, m1 * 2, m2 * 2)

    
    U_exact = exact_solution(X, Y)

    error_1 = np.max(np.abs(U_exact - U_num1))
    error_2 = np.max(np.abs(U_exact - U_num1))
    print(f"E2h/E={error_2 / error_1}")


    U_num = (4.0 * U_num2[::2,::2] - U_num1) / 3.0

    # 输出对应点的数值
    test_points = [(0.5, 0.25), (1.0, 0.25), (1.5, 0.25), (0.5, 0.5), (1.0, 0.5)]
    
    print(f"{'h1, h2':<12} | {'数值解':<14} | {'绝对误差'}")
